...
-- receive anonymous arg taking 'Quantity'
instance EdhAllocator fn' => EdhAllocator (Quantity -> fn') where
  allocEdhObj !fn (ArgsPack (val : args) !kwargs) !exit = case val of
    EdhQty !val' -> allocEdhObj (fn val') (ArgsPack args kwargs) exit
    _ -> throwEdhTx UsageError "arg type mismatch: anonymous"
  allocEdhObj _ _ _ = throwEdhTx UsageError "missing anonymous arg"

-- receive optional anonymous arg taking 'Quantity'
instance EdhAllocator fn' => EdhAllocator (Maybe Quantity -> fn') where
  allocEdhObj !fn (ArgsPack [] !kwargs) !exit =
    allocEdhObj (fn Nothing) (ArgsPack [] kwargs) exit
  allocEdhObj !fn (ArgsPack (val : args) !kwargs) !exit = case val of
    EdhQty !val' ->
...
-- receive anonymous arg taking 'BChan'
instance EdhAllocator fn' => EdhAllocator (BChan -> fn') where
  allocEdhObj !fn (ArgsPack (val : args) !kwargs) !exit = case val of
    EdhChan !val' -> allocEdhObj (fn val') (ArgsPack args kwargs) exit
    _ -> throwEdhTx UsageError "arg type mismatch: anonymous"
  allocEdhObj _ _ _ = throwEdhTx UsageError "missing anonymous arg"

-- receive optional anonymous arg taking 'BChan'
instance EdhAllocator fn' => EdhAllocator (Maybe BChan -> fn') where
  allocEdhObj !fn (ArgsPack [] !kwargs) !exit =
    allocEdhObj (fn Nothing) (ArgsPack [] kwargs) exit
  allocEdhObj !fn (ArgsPack (val : args) !kwargs) !exit = case val of
    EdhChan !val' -> allocEdhObj (fn (Just val')) (ArgsPack args kwargs) exit
    _ -> throwEdhTx UsageError "arg type mismatch: anonymous"
...
-- receive named arg taking 'Quantity'
instance (KnownSymbol name, EdhAllocator fn') => EdhAllocator (NamedEdhArg Quantity name -> fn') where
  allocEdhObj !fn (ArgsPack !args !kwargs) !exit =
    case odTakeOut (AttrByName argName) kwargs of
      (Just !val, !kwargs') -> case val of
        EdhQty !val' ->
...
-- receive named, optional arg taking 'Quantity'
instance (KnownSymbol name, EdhAllocator fn') => EdhAllocator (NamedEdhArg (Maybe Quantity) name -> fn') where
  allocEdhObj !fn (ArgsPack !args !kwargs) !exit =
    case odTakeOut (AttrByName argName) kwargs of
      (Just !val, !kwargs') -> case val of
        EdhQty !val' ->
...
-- receive named arg taking 'BChan'
instance (KnownSymbol name, EdhAllocator fn') => EdhAllocator (NamedEdhArg BChan name -> fn') where
  allocEdhObj !fn (ArgsPack !args !kwargs) !exit =
    case odTakeOut (AttrByName argName) kwargs of
      (Just !val, !kwargs') -> case val of
        EdhChan !val' ->
...
-- receive named, optional arg taking 'BChan'
instance (KnownSymbol name, EdhAllocator fn') => EdhAllocator (NamedEdhArg (Maybe BChan) name -> fn') where
  allocEdhObj !fn (ArgsPack !args !kwargs) !exit =
    case odTakeOut (AttrByName argName) kwargs of
      (Just !val, !kwargs') -> case val of
        EdhChan !val' ->
...
-- receive anonymous arg taking 'Quantity'
instance EdhCallable fn' => EdhCallable (Quantity -> fn') where
  callFromEdh !fn (ArgsPack (val : args) !kwargs) !exit = case val of
    EdhQty !val' -> callFromEdh (fn val') (ArgsPack args kwargs) exit
    _ -> throwEdhTx UsageError "arg type mismatch: anonymous"
  callFromEdh _ _ _ = throwEdhTx UsageError "missing anonymous arg"

-- receive optional anonymous arg taking 'Quantity'
instance EdhCallable fn' => EdhCallable (Maybe Quantity -> fn') where
  callFromEdh !fn (ArgsPack [] !kwargs) !exit =
    callFromEdh (fn Nothing) (ArgsPack [] kwargs) exit
  callFromEdh !fn (ArgsPack (val : args) !kwargs) !exit = case val of
    EdhQty !val' ->
...
-- receive anonymous arg taking 'BChan'
instance EdhCallable fn' => EdhCallable (BChan -> fn') where
  callFromEdh !fn (ArgsPack (val : args) !kwargs) !exit = case val of
    EdhChan !val' -> callFromEdh (fn val') (ArgsPack args kwargs) exit
    _ -> throwEdhTx UsageError "arg type mismatch: anonymous"
  callFromEdh _ _ _ = throwEdhTx UsageError "missing anonymous arg"

-- receive optional anonymous arg taking 'BChan'
instance EdhCallable fn' => EdhCallable (Maybe BChan -> fn') where
  callFromEdh !fn (ArgsPack [] !kwargs) !exit =
    callFromEdh (fn Nothing) (ArgsPack [] kwargs) exit
  callFromEdh !fn (ArgsPack (val : args) !kwargs) !exit = case val of
    EdhChan !val' -> callFromEdh (fn (Just val')) (ArgsPack args kwargs) exit
    _ -> throwEdhTx UsageError "arg type mismatch: anonymous"
...
-- receive named arg taking 'Quantity'
instance (KnownSymbol name, EdhCallable fn') => EdhCallable (NamedEdhArg Quantity name -> fn') where
  callFromEdh !fn (ArgsPack !args !kwargs) !exit =
    case odTakeOut (AttrByName argName) kwargs of
      (Just !val, !kwargs') -> case val of
        EdhQty !val' ->
...
-- receive named, optional arg taking 'Quantity'
instance (KnownSymbol name, EdhCallable fn') => EdhCallable (NamedEdhArg (Maybe Quantity) name -> fn') where
  callFromEdh !fn (ArgsPack !args !kwargs) !exit =
    case odTakeOut (AttrByName argName) kwargs of
      (Just !val, !kwargs') -> case val of
        EdhQty !val' ->
...
-- receive named arg taking 'BChan'
instance (KnownSymbol name, EdhCallable fn') => EdhCallable (NamedEdhArg BChan name -> fn') where
  callFromEdh !fn (ArgsPack !args !kwargs) !exit =
    case odTakeOut (AttrByName argName) kwargs of
      (Just !val, !kwargs') -> case val of
        EdhChan !val' ->
...
-- receive named, optional arg taking 'BChan'
instance (KnownSymbol name, EdhCallable fn') => EdhCallable (NamedEdhArg (Maybe BChan) name -> fn') where
  callFromEdh !fn (ArgsPack !args !kwargs) !exit =
    case odTakeOut (AttrByName argName) kwargs of
      (Just !val, !kwargs') -> case val of
        EdhChan !val' ->
